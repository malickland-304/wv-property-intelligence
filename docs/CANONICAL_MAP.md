...
| Production branch | `origin/main` | Verify current head with `git fetch origin --prune && git rev-parse origin/main`. Live runtime deploy is `24ee74b` from #113; repo-safe runtime and docs commits may advance `origin/main` before the next manual VPS deploy |
