...
- [ ] **Railway twin decommission (in progress)** — audit ✅ (Railway `contacts`/`leads` == VPS, hashes match, no un-migrated leads), full `/data` backup ✅ (downloaded to `~/railway-decommission-backup-2026-06-17/`, retain 30–90 days), service ✅ **stopped** (all deployments REMOVED). See `DECISIONS.md` 2026-06-17. **Remaining:** (1) disconnect the service's GitHub auto-deploy trigger so a `main` merge can't revive it (Railway dashboard); (2) after the retention window, hard-delete the service/volume; (3) remove `railway.json` + the legacy GitHub deployment environments from the repo; (4) rotate/retire the twin's copy of production secrets (incl. the old `ADMIN_PASSWORD`). ⚠️ **Human-only** (real lead PII + prod secrets). — **Phil**
- [ ] **Persist `listings/` uploads on the VPS** — admin photo uploads write to the container's `/workspace/listings`, which is **not** on a persistent volume in the current compose (only `wv-data` → `/data` is), so they are lost on redeploy / `--force-recreate` (Railway previously had a volume for this). Add a named volume/bind for `listings/` and/or confirm the Google Drive media backup is configured on the VPS before relying on local uploads. — no deps — **Phil / Claude Code**
