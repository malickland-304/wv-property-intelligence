...
Manual deploy (operator, SSH):
    git -C /docker/wv-property-intelligence/src fetch && checkout <sha>
...
- Photos stored on the container's `listings/` path (same host); migrate to S3+CDN is the upgrade path
